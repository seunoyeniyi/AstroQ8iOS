...
class CheckoutAddressViewController: NoBarViewController {
...
//                self.postCode.text = address["shipping_postcode"].stringValue
                ...
                //set default country
                if address["shipping_country"].stringValue.isEmpty {
                    self.setDefaultCountry(name: "Kuwait", code: "KW")
                }
    ...
    func setDefaultCountry(name: String, code: String) {
        self.country.text = name
        self.country.isUserInteractionEnabled = false
        let dStates = getCountryStates(countryCode: code, states: self.states)
        self.countryStates = dStates
        self.statesString = []
        for (_, state) in dStates.enumerated() {
            self.statesString.append(state["name"]!)
        }
        self.state.optionArray = self.statesString
    }
...
//            "selected_state": self.selectedState as AnyObject,
        ...
        var url = Site.init().UPDATE_CART_SHIPPING + userSession.ID;
        if userSession.logged() {
...
//                        self.presentationDelegate?.presentationDismissed(action: "present_payment", data: JSON(parameters))
//                    })
//                } else {
...
//                paymentViewController.paymentDelegate = self
                self.paymentViewController.parseData = JSON(parameters)
                self.presentWithCondition(controller: self.paymentViewController, animated: true, completion: nil)
            } else {
