...
//    var selected_kid_size: String = "" //not needed in shop
    ...
    func fetchProducts(sort: String, category: String, tag: String, priceRange: Array<String>, paged: Int, shim: Bool, emptyProducts: Bool = false) {
        ...
        if (priceRange.count > 1) {
            url = Site.init().SIMPLE_PRODUCTS + "?per_page=20" + "&paged=\(paged)" + "&hide_description=1&show_variation=1&user_id=" + userSession.ID + "&price_range=" + priceRange[0] + "," + priceRange[1]
        }
        if (self.selected_size.count > 0) {
...
extension ShopViewController: AddToCartDelegate, AddedToCartDialogDelegate {
...
extension ShopViewController: AttributesDialogDelegate {
...
//        self.selected_kid_size = kid_size //not needed in shop
