                    ...
                    if (items.count < 1) {
                        ...
                        for (_, item): (String, JSON) in items {
                            self.cartProducts.append([
                                ...
                                "subtotal": item["subtotal"].stringValue,
                                "rules_applied": item["rules_applied"].stringValue
                                ])
                        }
                        
                        DispatchQueue.main.async {
                            self.cartTableView.reloadData()
                            self.cartTableViewHeightC.constant = CGFloat(119 * self.cartProducts.count)
                        ...
                        if (json["has_coupon"].stringValue == "true") {
                            couponDiscount = json["coupon_discount"].doubleValue
...
extension CartViewController: UITableViewDelegate, UITableViewDataSource, CartTableDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.titleLbl.text = self.cartProducts[indexPath.row]["product_title"]?.htmlToString
        ...
        if (self.cartProducts[indexPath.row]["rules_applied"]! == "true") {
            cell.discountPriceLabel.text = Site.init().CURRENCY + PriceFormatter.format(price: self.cartProducts[indexPath.row]["subtotal"]!);
            let attributeString: NSMutableAttributedString = NSMutableAttributedString(string: Site.init().CURRENCY +  self.cartProducts[indexPath.row]["price"]!)
            ...
            if self.cartProducts[indexPath.row]["subtotal"]! == "0" {
                cell.discountStatusLabel.isHidden = false
                cell.discountStatusLabel.text = "Free"
            }
...
//        self.cartProducts[index]["quantity"] = String(quantity)
        ...
        self.theCart.addToCart(productID: productID, quantity: quantity, replaceQuantity: true, controller: self)
    }
